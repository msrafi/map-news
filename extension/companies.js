// Generated from src/data/companies.json by `npm run sync:places`. Do not edit by hand.
// US companies whose headlines are kept even when no place is named.
self.MAP_NEWS_COMPANIES = [
  {"ticker":"AAPL","names":["apple"]},
  {"ticker":"MSFT","names":["microsoft"]},
  {"ticker":"GOOGL","names":["alphabet","google"]},
  {"ticker":"AMZN","names":["amazon"]},
  {"ticker":"META","names":["meta platforms","meta","facebook"]},
  {"ticker":"NVDA","names":["nvidia"]},
  {"ticker":"TSLA","names":["tesla"]},
  {"ticker":"ORCL","names":["oracle"]},
  {"ticker":"AMD","names":["advanced micro devices","amd"]},
  {"ticker":"INTC","names":["intel"]},
  {"ticker":"AVGO","names":["broadcom"]},
  {"ticker":"MU","names":["micron"]},
  {"ticker":"QCOM","names":["qualcomm"]},
  {"ticker":"TXN","names":["texas instruments"]},
  {"ticker":"CRM","names":["salesforce"]},
  {"ticker":"ADBE","names":["adobe"]},
  {"ticker":"IBM","names":["ibm"]},
  {"ticker":"CSCO","names":["cisco"]},
  {"ticker":"NFLX","names":["netflix"]},
  {"ticker":"DIS","names":["disney"]},
  {"ticker":"UBER","names":["uber"]},
  {"ticker":"PLTR","names":["palantir"]},
  {"ticker":"COIN","names":["coinbase"]},
  {"ticker":"MSTR","names":["microstrategy","strategy inc"]},
  {"ticker":"BA","names":["boeing"]},
  {"ticker":"LMT","names":["lockheed martin","lockheed"]},
  {"ticker":"RTX","names":["raytheon","rtx corp"]},
  {"ticker":"GE","names":["general electric"]},
  {"ticker":"CAT","names":["caterpillar"]},
  {"ticker":"F","names":["ford motor","ford"]},
  {"ticker":"GM","names":["general motors"]},
  {"ticker":"XOM","names":["exxon mobil","exxon","exxonmobil"]},
  {"ticker":"CVX","names":["chevron"]},
  {"ticker":"COP","names":["conocophillips"]},
  {"ticker":"OXY","names":["occidental petroleum","occidental"]},
  {"ticker":"SLB","names":["schlumberger"]},
  {"ticker":"JPM","names":["jpmorgan","jp morgan","j.p. morgan"]},
  {"ticker":"BAC","names":["bank of america"]},
  {"ticker":"WFC","names":["wells fargo"]},
  {"ticker":"C","names":["citigroup"]},
  {"ticker":"GS","names":["goldman sachs","goldman"]},
  {"ticker":"MS","names":["morgan stanley"]},
  {"ticker":"BLK","names":["blackrock"]},
  {"ticker":"BRK.B","names":["berkshire hathaway","berkshire"]},
  {"ticker":"V","names":["visa"]},
  {"ticker":"MA","names":["mastercard"]},
  {"ticker":"PYPL","names":["paypal"]},
  {"ticker":"WMT","names":["walmart"]},
  {"ticker":"COST","names":["costco"]},
  {"ticker":"TGT","names":["target corp"]},
  {"ticker":"HD","names":["home depot"]},
  {"ticker":"NKE","names":["nike"]},
  {"ticker":"SBUX","names":["starbucks"]},
  {"ticker":"MCD","names":["mcdonald's","mcdonalds"]},
  {"ticker":"KO","names":["coca-cola","coca cola"]},
  {"ticker":"PEP","names":["pepsico","pepsi"]},
  {"ticker":"PG","names":["procter & gamble","procter and gamble"]},
  {"ticker":"JNJ","names":["johnson & johnson"]},
  {"ticker":"PFE","names":["pfizer"]},
  {"ticker":"MRK","names":["merck"]},
  {"ticker":"LLY","names":["eli lilly"]},
  {"ticker":"ABBV","names":["abbvie"]},
  {"ticker":"MRNA","names":["moderna"]},
  {"ticker":"UNH","names":["unitedhealth"]},
  {"ticker":"CVS","names":["cvs health"]},
  {"ticker":"T","names":["at&t"]},
  {"ticker":"VZ","names":["verizon"]},
  {"ticker":"CMCSA","names":["comcast"]},
  {"ticker":"DAL","names":["delta air lines"]},
  {"ticker":"UAL","names":["united airlines"]},
  {"ticker":"AAL","names":["american airlines"]},
  {"ticker":"LUV","names":["southwest airlines"]},
  {"ticker":"FDX","names":["fedex"]},
  {"ticker":"UPS","names":["united parcel","ups inc"]},
  {"ticker":"NEE","names":["nextera energy","nextera"]},
  {"ticker":"VST","names":["vistra"]},
  {"ticker":"CEG","names":["constellation energy"]},
  {"ticker":"SMCI","names":["super micro","supermicro"]},
  {"ticker":"DELL","names":["dell technologies","dell"]},
  {"ticker":"HPQ","names":["hewlett-packard","hp inc"]},
  {"ticker":"ASTS","names":["ast spacemobile"]},
  {"ticker":"RIVN","names":["rivian"]},
  {"ticker":"LCID","names":["lucid group","lucid motors"]},
  {"ticker":"GME","names":["gamestop"]},
  {"ticker":"AMC","names":["amc entertainment"]},
  {"ticker":"TDOC","names":["teladoc"]},
  {"ticker":"SPY","names":["spdr s&p 500"]},
]
